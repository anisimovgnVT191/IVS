...
class CommandRMC(commandStr: String): CommandNMEA(commandStr){
    ...
    val status: Char = commandsList[2]!![0]
    override val latitude: Double = commandsList[3]!!.toDouble()
    override val indicatorNS: Direction = commandsList[4]!!.toDirection()
    override val longitude: Double = commandsList[5]!!.toDouble()
    override val indicatorEW: Direction = commandsList[6]!!.toDirection()
    val speedOverGround: Double = commandsList[7]?.toDouble()?:0.0
    val courseOverGround: Double = commandsList[8]!!.toDouble()
    val date: Date by lazy { getDate(9) }
    val magneticVariationDegress: Double? = commandsList[10]?.toDouble()
    val magneticVariationDiraction: Direction?  = commandsList[11]?.toDirection()
    val modeIndicator: Char = commandsList[12]!!.split("*")[0][0]
    val checksum: Int = commandsList[12]!!.split("*")[1].toInt(16)
